...
Font::Font(File file, float size, bool preload) : file(file), size(size), preloaded(preload) {

	this->atlas = ftgl::texture_atlas_new(512, 512, 1);
	this->font = ftgl::texture_font_new_from_file(this->atlas, this->size, file.getAbsolutePath().c_str());
